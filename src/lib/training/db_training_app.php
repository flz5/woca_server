...
include_once "appstruct_training.php";

function db_training_getAppStructAll(){
    ...
    foreach($result->fetch_all(MYSQLI_ASSOC) as $dsatz){
        ...
        $stmt2 = $mysqli->prepare("select * from training_group WHERE id = ?");
        $stmt2->bind_param('i',$dsatz['training_group']);
        $stmt2->execute();
